...
func TestCheckColumnNames(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "CTE source flags unknown column",
			sql:              "WITH t AS (SELECT id FROM users) SELECT t.nme FROM t",
			expectedCount:    1,
			expectedCode:     unknownColumnCode,
			expectedCategory: diag.CategoryUnknownColumn,
			expectedMessage:  `"nme"`,
			expectedAvail:    []string{"id"},
		},
		{
			name:          "CTE source resolves valid column",
			sql:           "WITH t AS (SELECT id FROM users) SELECT t.id FROM t",
			expectedCount: 0,
		},
		{
			name:          "unqualified ref to CTE column resolves",
			sql:           "WITH t AS (SELECT id FROM users) SELECT id FROM t",
			expectedCount: 0,
		},
		{
			name:          "CTE explicit alias list overrides projection",
			sql:           "WITH t(a) AS (SELECT id FROM users) SELECT t.a FROM t",
			expectedCount: 0,
		},
		{
			name:             "CTE explicit alias list hides original column",
			sql:              "WITH t(a) AS (SELECT id FROM users) SELECT t.id FROM t",
			expectedCount:    1,
			expectedCode:     unknownColumnCode,
			expectedCategory: diag.CategoryUnknownColumn,
			expectedMessage:  `"id"`,
			expectedAvail:    []string{"a"},
		},
		{
			name:          "CTE with star skips column refs",
			sql:           "WITH t AS (SELECT * FROM users) SELECT t.anything FROM t",
			expectedCount: 0,
		},
		{
			name:             "FROM-subquery flags unknown column",
		...
		{
			name:          "FROM-subquery resolves valid column",
			sql:           "SELECT x.id FROM (SELECT id FROM users) x",
			expectedCount: 0,
		},
		{
			name:          "FROM-subquery explicit alias list overrides projection",
			sql:           "SELECT x.a FROM (SELECT id FROM users) x(a)",
			expectedCount: 0,
		},
		{
			name:             "set-op CTE inherits left branch projection",
			sql:              "WITH x AS (SELECT id FROM users UNION SELECT user_id FROM orders) SELECT x.foo FROM x",
			expectedCount:    1,
			expectedCode:     unknownColumnCode,
			expectedCategory: diag.CategoryUnknownColumn,
			expectedMessage:  `"foo"`,
		...
		{
			name:          "set-op CTE resolves left branch column",
			sql:           "WITH x AS (SELECT id FROM users UNION SELECT user_id FROM orders) SELECT x.id FROM x",
			expectedCount: 0,
		},
		{
			name:            "parenthesized CTE body still infers projection",
			sql:             "WITH t AS ((SELECT id FROM users)) SELECT t.foo FROM t",
			expectedCount:   1,
			expectedCode:    unknownColumnCode,
			expectedMessage: `"foo"`,
			expectedAvail:   []string{"id"},
		...
		{
			name:            "parenthesized FROM-subquery still infers projection",
			sql:             "SELECT x.foo FROM ((SELECT id FROM users)) x",
			expectedCount:   1,
			expectedCode:    unknownColumnCode,
			expectedMessage: `"foo"`,
		...
		{
			name:          "CTE alias case-insensitive on lookup",
			sql:           "WITH T AS (SELECT id FROM users) SELECT t.id FROM T",
			expectedCount: 0,
		},
		{
			name:            "CTE alias case-insensitive flags unknown column",
			sql:             "WITH T AS (SELECT id FROM users) SELECT T.foo FROM t",
			expectedCount:   1,
			expectedMessage: `"foo"`,
			expectedAvail:   []string{"id"},
		...
		{
			name:            "CTE shadows catalog table of same name",
			sql:             "WITH users AS (SELECT id FROM users) SELECT users.name FROM users",
			expectedCount:   1,
			expectedMessage: `"name"`,
			expectedAvail:   []string{"id"},
		},
		{
			name:          "CTE with anonymous expression defeats inference",
			sql:           "WITH t AS (SELECT 1, name FROM users) SELECT t.foo FROM t",
			expectedCount: 0,
		},
		{
			name:          "CTE projection with cast defeats inference",
			sql:           "WITH t AS (SELECT id::int FROM users) SELECT t.id FROM t",
			expectedCount: 0,
		},
		{
			name:          "CTE body with qualified projection resolves",
			sql:           "WITH t AS (SELECT u.id FROM users u) SELECT t.id FROM t",
			expectedCount: 0,
		},
		{
			name:            "CTE body with qualified projection flags unknown column",
			sql:             "WITH t AS (SELECT u.id FROM users u) SELECT t.u FROM t",
			expectedCount:   1,
			expectedCode:    unknownColumnCode,
			expectedMessage: `"u"`,
			expectedAvail:   []string{"id"},
		...
		{
			name:          "aliased opaque expression survives inference",
			sql:           "WITH t AS (SELECT id::int AS x FROM users) SELECT t.x FROM t",
			expectedCount: 0,
		},
		{
			name:            "aliased opaque expression flags unknown column",
			sql:             "WITH t AS (SELECT id::int AS x FROM users) SELECT t.foo FROM t",
			expectedCount:   1,
			expectedCode:    unknownColumnCode,
			expectedMessage: `"foo"`,
			expectedAvail:   []string{"x"},
		},
		{
